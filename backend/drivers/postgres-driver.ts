/**
 * PostgreSQL 驱动：全部 db/* 业务逻辑内化为类方法，纯函数辅助保留为模块函数。
 */
...
import type { PostgresSessionConnection } from "../session-connection";
import type { PoolClient } from "pg";
import Cursor from "pg-cursor";

/** 从 connectionMap 取当前会话并断言为 PostgreSQL（重试路径需取重建后的新会话） */
function pgSession(cid: string): PostgresSessionConnection {
...
export class PostgresDriver implements DbDriver<PostgresSessionConnection> {
  ...
  async connect(
    ...
    ctx: DriverContext,
  ): Promise<DbConnectOutcome<PostgresSessionConnection>> {
    ...
    return { session, result: { success: true, connectionId: cid, dbType: "postgres" as const } };
  }

  async disconnect(
    session: PostgresSessionConnection,
  ...
  async query(session: PostgresSessionConnection, params: ApiRequestPayload["db/query"]): Promise<unknown> {
    const cid = params.connectionId;
    const { query } = params;
    let lastErr: unknown;
    for (let attempt = 0; attempt < 2; attempt++) {
      const s = attempt === 0 ? session : pgSession(cid);
      const client = s.userUsedClient;
      if ((client as any).processID) s.runningQueryPid = (client as any).processID;
      try {
  ...
  async queryStream(session: PostgresSessionConnection, params: ApiRequestPayload["db/query-stream"]): Promise<unknown> {
    const cid = params.connectionId;
    ...
    for (let attempt = 0; attempt < 2; attempt++) {
      const s = attempt === 0 ? session : pgSession(cid);
      const client = s.userUsedClient;
      const adminPool = s.backGroundPool;
      try {
        if (s.cursor) {
          await new Promise<void>((r) => s.cursor!.instance.close(() => r()));
          s.cursor = undefined;
        }

        try {
          const pidRes = await client.query("SELECT pg_backend_pid() as pid");
          s.runningQueryPid = parseInt(String(pidRes.rows[0]?.pid ?? 0), 10) || undefined;
        } catch {
          s.runningQueryPid = (client as any).processID;
        }
        ...
        if (isDone) {
          ...
          s.cursor = { instance: cursor, columns: columnsInfo, isDone: false };
        }
  ...
  async queryStreamMore(
    session: PostgresSessionConnection,
    params: ApiRequestPayload["db/query-stream-more"],
  ): Promise<unknown> {
    const cid = params.connectionId;
  ...
  async cancelQuery(session: PostgresSessionConnection, _params: ApiRequestPayload["db/cancel-query"]): Promise<unknown> {
  ...
  async saveChanges(session: PostgresSessionConnection, params: ApiRequestPayload["db/save-changes"]): Promise<unknown> {
    const cid = params.connectionId;
    const result = await session.backGroundPool.query(params.sql);
    sendSSEMessage(cid, {
      type: "INFO",
  ...
  async importRows(session: PostgresSessionConnection, params: ApiRequestPayload["db/import-rows"]): Promise<unknown> {
    const cid = params.connectionId;
    const {
      schema,
      table,
  ...
  async explain(session: PostgresSessionConnection, params: ApiRequestPayload["db/explain"]): Promise<unknown> {
    const cid = params.connectionId;
    const { query } = params;
  ...
  async explainText(session: PostgresSessionConnection, params: ApiRequestPayload["db/explain-text"]): Promise<unknown> {
    try {
      const { query } = params;
      const q = query.trim();
  ...
  async getSchemas(session: PostgresSessionConnection, _params: ApiRequestPayload["db/schemas"]): Promise<unknown> {
  ...
  async getTables(session: PostgresSessionConnection, params: ApiRequestPayload["db/tables"]): Promise<unknown> {
    const { schema } = params;
    const result = await session.backGroundPool.query(
  ...
  async getColumns(session: PostgresSessionConnection, params: ApiRequestPayload["db/columns"]): Promise<unknown> {
  ...
  async getIndexes(session: PostgresSessionConnection, params: ApiRequestPayload["db/indexes"]): Promise<unknown> {
    const { schema, table } = params;
    const result = await session.backGroundPool.query(
      `SELECT
  ...
  async getPrimaryKeys(session: PostgresSessionConnection, params: ApiRequestPayload["db/primary-keys"]): Promise<unknown> {
    const { schema, table } = params;
    const result = await session.backGroundPool.query(
  ...
  async getUniqueConstraints(
    session: PostgresSessionConnection,
    params: ApiRequestPayload["db/unique-constraints"],
  ): Promise<unknown> {
    const { schema, table } = params;
    const result = await session.backGroundPool.query(
  ...
  async getCheckConstraints(
    session: PostgresSessionConnection,
    params: ApiRequestPayload["db/check-constraints"],
  ): Promise<unknown> {
    const { schema, table } = params;
    const result = await session.backGroundPool.query(
  ...
  async getForeignKeys(session: PostgresSessionConnection, params: ApiRequestPayload["db/foreign-keys"]): Promise<unknown> {
    const { schema, table } = params;
  ...
  async getDataTypes(session: PostgresSessionConnection, _params: ApiRequestPayload["db/data-types"]): Promise<unknown> {
  ...
  async executeDdl(session: PostgresSessionConnection, params: ApiRequestPayload["db/execute-ddl"]): Promise<unknown> {
    const cid = params.connectionId;
    const { sql } = params;
    try {
  ...
  async getTableDdl(session: PostgresSessionConnection, params: ApiRequestPayload["db/table-ddl"]): Promise<unknown> {
    const { schema, table } = params;
  ...
  async getFunctionDdl(session: PostgresSessionConnection, params: ApiRequestPayload["db/function-ddl"]): Promise<unknown> {
    const { schema, function: funcName, oid } = params;
    const pool = session.backGroundPool;
  ...
  async getTableComment(session: PostgresSessionConnection, params: ApiRequestPayload["db/table-comment"]): Promise<unknown> {
    const { schema, table } = params;
    const result = await session.backGroundPool.query(
  ...
  async schemaDump(session: PostgresSessionConnection, params: ApiRequestPayload["db/schema-dump"]): Promise<unknown> {
    const cid = params.connectionId;
    const { schema, includeData = false } = params;
    const pool = session.backGroundPool;
    const quote = (s: string) => `"${s.replace(/"/g, '""')}"`;
    ...
    for (const t of sortedTables) {
      const { ddl } = (await this.getTableDdl(session, { connectionId: cid, dbType: session.dbKind, schema, table: t })) as { ddl: string };
    ...
    for (const r of viewsRes.rows as any[]) {
      ...
      const { ddl } = (await this.getTableDdl(session, { connectionId: cid, dbType: session.dbKind, schema, table: v })) as { ddl: string };
      parts.push("\n" + ddl + (ddl.trim().endsWith(";") ? "\n" : ";\n"));
    }
    for (const r of funcsRes.rows as any[]) {
      const { ddl } = (await this.getFunctionDdl(session, { connectionId: cid, dbType: session.dbKind, schema, function: (r as any).proname, oid: (r as any).oid })) as { ddl: string };
      parts.push("\n" + ddl + (ddl.trim().endsWith(";") ? "\n" : ";\n"));
    }
  ...
  async databaseDump(session: PostgresSessionConnection, params: ApiRequestPayload["db/database-dump"]): Promise<unknown> {
    const cid = params.connectionId;
    ...
    for (const r of schemasRes.rows as any[]) {
      const s = r.nspname;
      const { dump } = (await this.schemaDump(session, { connectionId: cid, dbType: session.dbKind, schema: s, includeData })) as { dump: string };
      parts.push(dump);
    }
  ...
  async getPartitionInfo(session: PostgresSessionConnection, params: ApiRequestPayload["db/partition-info"]): Promise<unknown> {
    try {
      const { schema, table } = params;
      const pool = session.backGroundPool;
  ...
  async sessionMonitor(session: PostgresSessionConnection, params: ApiRequestPayload["db/session-monitor"]): Promise<unknown> {
    try {
  ...
  async sessionControl(session: PostgresSessionConnection, params: ApiRequestPayload["db/session-control"]): Promise<unknown> {
    try {
