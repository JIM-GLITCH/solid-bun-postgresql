/**
 * MySQL / MariaDB 驱动：全部 db/* 业务逻辑内化为类方法，纯函数辅助保留为模块函数。
 * MySQL 与 MariaDB 共用 mysql2 协议，故一个驱动覆盖两种方言。
 */
...
import type { MysqlSessionConnection } from "../session-connection";

/** 从 connectionMap 取当前会话并断言为 MySQL/MariaDB */
function mysqlSession(cid: string): MysqlSessionConnection {
...
export class MysqlDriver implements DbDriver<MysqlSessionConnection> {
  readonly kind = "mysql" as const;
  readonly kinds = ["mysql", "mariadb"] as const;
  ...
  async connect(
    ...
    ctx: DriverContext,
  ): Promise<DbConnectOutcome<MysqlSessionConnection>> {
    ...
    return { session, result: { success: true, connectionId: cid, dbType: mysqlFamilyKind } };
  }
  ...
  async query(session: MysqlSessionConnection, params: ApiRequestPayload["db/query"]): Promise<unknown> {
    const cid = params.connectionId;
    const { query, defaultSchema } = params;
  ...
  async queryStream(session: MysqlSessionConnection, params: ApiRequestPayload["db/query-stream"]): Promise<unknown> {
    const cid = params.connectionId;
    const { query, statements: payloadStatements, batchSize = 100, defaultSchema } = params;
  ...
  async queryStreamMore(
    ...
    const cid = params.connectionId;
    const { batchSize = 100, defaultSchema } = params;
    const bs = Math.max(1, batchSize ?? 100);
  ...
  async cancelQuery(session: MysqlSessionConnection, _params: ApiRequestPayload["db/cancel-query"]): Promise<unknown> {
  ...
  async saveChanges(session: MysqlSessionConnection, params: ApiRequestPayload["db/save-changes"]): Promise<unknown> {
    const cid = params.connectionId;
    const [res] = await session.backGroundPool.query(params.sql);
    const affected =
  ...
  async importRows(session: MysqlSessionConnection, params: ApiRequestPayload["db/import-rows"]): Promise<unknown> {
    const cid = params.connectionId;
    const {
      schema,
      table,
  ...
  async explain(session: MysqlSessionConnection, params: ApiRequestPayload["db/explain"]): Promise<unknown> {
    const cid = params.connectionId;
    const { query, defaultSchema } = params;
    const q = query.trim();
  ...
  async explainText(session: MysqlSessionConnection, params: ApiRequestPayload["db/explain-text"]): Promise<unknown> {
    const cid = params.connectionId;
    const { query } = params;
    const q = query.trim();
  ...
  async getSchemas(session: MysqlSessionConnection, _params: ApiRequestPayload["db/schemas"]): Promise<unknown> {
  ...
  async getTables(session: MysqlSessionConnection, params: ApiRequestPayload["db/tables"]): Promise<unknown> {
    const { schema } = params;
  ...
  async getColumns(session: MysqlSessionConnection, params: ApiRequestPayload["db/columns"]): Promise<unknown> {
  ...
  async getIndexes(session: MysqlSessionConnection, params: ApiRequestPayload["db/indexes"]): Promise<unknown> {
    const { schema, table } = params;
  ...
  async getPrimaryKeys(session: MysqlSessionConnection, params: ApiRequestPayload["db/primary-keys"]): Promise<unknown> {
    const { schema, table } = params;
  ...
  async getForeignKeys(session: MysqlSessionConnection, params: ApiRequestPayload["db/foreign-keys"]): Promise<unknown> {
    const { schema, table } = params;
  ...
  async getDataTypes(session: MysqlSessionConnection, _params: ApiRequestPayload["db/data-types"]): Promise<unknown> {
  ...
  async executeDdl(session: MysqlSessionConnection, params: ApiRequestPayload["db/execute-ddl"]): Promise<unknown> {
    const cid = params.connectionId;
    const { sql } = params;
    try {
      sendSSEMessage(cid, {
  ...
  async getTableDdl(session: MysqlSessionConnection, params: ApiRequestPayload["db/table-ddl"]): Promise<unknown> {
    const { schema, table } = params;
    const pool = session.backGroundPool;
  ...
  async getFunctionDdl(session: MysqlSessionConnection, params: ApiRequestPayload["db/function-ddl"]): Promise<unknown> {
    const { schema, function: funcName } = params;
    const pool = session.backGroundPool;
  ...
  async getTableComment(session: MysqlSessionConnection, params: ApiRequestPayload["db/table-comment"]): Promise<unknown> {
    const { schema, table } = params;
    const [rows] = await session.backGroundPool.query(
  ...
  async schemaDump(session: MysqlSessionConnection, params: ApiRequestPayload["db/schema-dump"]): Promise<unknown> {
    const cid = params.connectionId;
  ...
  async databaseDump(session: MysqlSessionConnection, params: ApiRequestPayload["db/database-dump"]): Promise<unknown> {
    const cid = params.connectionId;
    ...
    for (const dbName of names) {
      const { dump } = (await this.schemaDump(session, { connectionId: cid, dbType: session.dbKind, schema: dbName, includeData })) as { dump: string };
      parts.push(dump);
    }
  ...
  async getPartitionInfo(session: MysqlSessionConnection, params: ApiRequestPayload["db/partition-info"]): Promise<unknown> {
    const { schema, table } = params;
  ...
  async sessionMonitor(session: MysqlSessionConnection, params: ApiRequestPayload["db/session-monitor"]): Promise<unknown> {
    const cid = params.connectionId;
  ...
  async sessionControl(session: MysqlSessionConnection, params: ApiRequestPayload["db/session-control"]): Promise<unknown> {
  ...
  async installedExtensions(
    _session: MysqlSessionConnection,
