...
| `partitionStructureInspect` | 侧栏「分区结构 / 裁剪预览」：`db/partition-info` + 文本 EXPLAIN | `sidebar.tsx`                         |
...
| `streamingQuery`     | ✅          | ✅               | ✅          |
| `cancelQuery`        | ✅          | ✅               | ✅          |
| `explainAnalyzeJson` | ✅          | ✅               | ✅（SHOWPLAN_XML 字符串） |
| `explainText`        | ✅          | ✅               | ✅（SHOWPLAN_ALL） |
| `sessionMonitor`     | ✅          | ✅               | ✅          |
| `pgExtensionCatalog` | ✅          | ❌               | ❌          |
| `tableDesigner`      | ✅          | ✅               | ✅          |
| `resultCellEdit`     | ✅          | ✅               | ✅          |
...
| 查看 DDL         | `db/table-ddl`                                                          | ✅   | ✅       | ✅          |                         |
| 分区结构           | `db/partition-info`、文本 EXPLAIN 等；入口受 `partitionStructureInspect` 控制              | ✅   | ✅       | ✅          |                         |
| 清空表 / 删表       | 前端按方言拼 `DROP`/`TRUNCATE` → `db/execute-ddl`                              | ✅   | ✅       | ✅          | 标识符：PG `"`、MySQL `` ` ``、T-SQL `[]` |
| SELECT / COUNT | 前端 SQL → `db/query-stream`（及侧栏等流式路径）                                  | ✅   | ✅       | ✅          |                         |
...
| SQL Server | ✅ `sql.Transaction` + 参数化批量 `INSERT` / `MERGE` | ✅ `MERGE`（`conflictColumns` + `onConflict`） |
