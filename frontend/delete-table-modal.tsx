...
export default function DeleteTableModal(props: DeleteTableModalProps) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      await executeDdl(props.connectionId, `DROP TABLE "${schema}"."${table}";`);
  ...
  return (
    ...
    >
      ...
      >
        <h2 id="delete-modal-title" style={{ "font-size": "16px", margin: "0 0 12px 0", color: vscode.error }}>
          删除表
        </h2>
        <p style={{ "font-size": "13px", color: vscode.foreground, margin: "0 0 16px 0" }}>
          确定要删除表 <strong style={{ "font-family": "'JetBrains Mono', monospace" }}>{props.schema}.{props.table}</strong> 吗？此操作不可恢复。
        </p>
        <Show when={error()}>
