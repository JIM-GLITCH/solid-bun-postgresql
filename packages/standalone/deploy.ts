...
async function main() {
  ...
  await Bun.$`bun packages/standalone/build-linux.ts`.catch((e: Error) => {
    console.error(e);
  });
  console.log("✅ 编译完成\n");
  ...
  await Bun.$`scp -i ${ECS_KEY} ./packages/standalone/out/server-linux root@${ECS_HOST}:/services/server-linux`;
  console.log("✅ 上传完成\n");
